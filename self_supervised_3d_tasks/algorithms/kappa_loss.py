...
# Metrics by https://github.com/benhamner/Metrics
...
def weighted_mean_quadratic_weighted_kappa(solution, submission):
    ...
    kappas = [
        quadratic_weighted_kappa(group[1]["essay_score"], group[1]["predicted_score"])
        for group in groups
    ]
...
def test_quadratic_kappa_website():
    import math

    actuals = np.array([4, 4, 3, 4, 4, 4, 1, 1, 2, 1])
    preds = np.array([0, 2, 1, 0, 0, 0, 1, 1, 2, 1])
    kappa = quadratic_weighted_kappa(actuals, preds, min_rating=0, max_rating=4)
    assert math.isclose(kappa, -0.139240506329114, rel_tol=1e-5)


def test_quadratic_kappa_equal():
    import math

    actuals = np.array([1, 1, 1, 1, 0, 0])
    preds = np.array([1, 1, 1, 1, 0, 0])
    kappa = quadratic_weighted_kappa(actuals, preds, min_rating=0, max_rating=4)
    assert math.isclose(kappa, 1.0, rel_tol=1e-5)


def test_quadratic_kappa_furthest():
    import math
    actuals = np.array([0 for x in range(1000)])
    preds = np.array([4 for x in range(1000)])
    kappa = quadratic_weighted_kappa(actuals, preds, min_rating=0, max_rating=4)
    assert math.isclose(kappa, -1.0, rel_tol=1e-5), f"actual kappa:{kappa}"
