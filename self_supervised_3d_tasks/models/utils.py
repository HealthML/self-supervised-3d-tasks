...
from self_supervised_3d_tasks.models import (
    vggnet, resnet, unet_resnet_2d, unet_resnet_3d
)


def get(architecture, **kwargs):
    ...
    if 'vgg19' == architecture:
        net = functools.partial(
            vggnet.vgg19,
            filters_factor=kwargs.get('filters_factor', 8))
    else:
        if 'resnet50' == architecture:
            ...
            assert kwargs.task == 'supervised_segmentation', 'unet_resnet50 is usable only with supervised_segmentation task.'
            net = unet_resnet_2d.unet_resnet50
        elif 'unet_resnet34' == architecture:
            assert kwargs.task == 'supervised_segmentation', 'unet_resnet34 is usable only with supervised_segmentation task.'
            net = unet_resnet_2d.unet_resnet34
        elif 'unet_resnet18' == architecture:
            assert kwargs.task == 'supervised_segmentation', 'unet_resnet18 is usable only with supervised_segmentation task.'
            ...
            assert kwargs.task == 'supervised_segmentation', 'unet_resnet3d is usable only with supervised_segmentation task.'
    ...
    net = functools.partial(
        net, num_classes=kwargs.get("num_classes", None),
