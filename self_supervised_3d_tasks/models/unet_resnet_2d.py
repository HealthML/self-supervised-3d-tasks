...
def bottleneck_v1_block(x, filters, training,  # pylint: disable=missing-docstring
                        strides=1,
...
def residual_block(x, filters, training,  # pylint: disable=missing-docstring
                   strides=1,
                   activation_fn=tf.nn.relu,
                   normalization_fn=batch_norm,
                   kernel_regularizer=None):
...
def unet_resnet(x,  # pylint: disable=missing-docstring
    ...
    with tf.variable_scope("decoder"):
        x = decoder_block(x, skip=end_points['block4'], filters=end_points['block5_filters'], training=is_training)
        end_points['block5_up'] = x

        x = decoder_block(x, skip=end_points['block3'], filters=end_points['block4_filters'], training=is_training)
        end_points['block4_up'] = x

        x = decoder_block(x, skip=end_points['block2'], filters=end_points['block3_filters'], training=is_training)
        end_points['block3_up'] = x

        x = decoder_block(x, skip=end_points['block1'], filters=end_points['block2_filters'], training=is_training)
...
unet_resnet50 = functools.partial(unet_resnet, num_encoder_layers=(3, 4, 6, 3, 3), block=bottleneck_v1_block)
unet_resnet34 = functools.partial(unet_resnet, num_encoder_layers=(3, 4, 6, 3, 3), block=residual_block)
unet_resnet18 = functools.partial(unet_resnet, num_encoder_layers=(2, 2, 2, 2, 2), block=residual_block)
