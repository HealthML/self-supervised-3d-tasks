...
class ExemplarBuilder:
    ...
    def apply_model(self, input_shape, **kwargs):
        ...
        """
        # defines encoder
        network = apply_encoder_model_3d((*self.dim, self.number_channels), self.embedding_size, **self.kwargs) if self.dim_3d else apply_encoder_model((*self.dim, self.number_channels), self.embedding_size, **self.kwargs)
