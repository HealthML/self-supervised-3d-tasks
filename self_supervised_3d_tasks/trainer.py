...
def make_estimator(mode, loss=None, eval_metrics=None, predictions=None, common_hooks=None, train_hooks=None):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        if common_hooks is not None:
            ...
            return spec_type(mode=mode, loss=loss, eval_metrics=eval_metrics, evaluation_hooks=common_hooks)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if common_hooks is not None:
            ...
            return spec_type(mode=mode, loss=loss, train_op=train_op, training_hooks=common_hooks)
        if common_hooks is not None:
            if not isinstance(common_hooks, list):
                common_hooks = [common_hooks]
            return spec_type(mode=mode, loss=loss, train_op=train_op, training_hooks=common_hooks)
